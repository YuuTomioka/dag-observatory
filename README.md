...
- 例: `{service="dag-observatory-dag-core", env="dev", app="dag-observatory-dag-core"}`
...
1) Grafana Explore で Tempo を選択し、`service.name=dag-observatory-dag-core` などで検索  
2) 目的の trace を開き、Span 詳細から `trace_id` を確認  
3) Loki に切り替え、`{service="dag-observatory-dag-core", env="dev"}` に `|= "trace_id=<id>"` を追加して意図ログを確認  
